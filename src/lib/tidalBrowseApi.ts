...
export async function fetchTidalHotNewSections() {
  ...
  const sections = await Promise.all(selectedModules.map((module) => buildSection(module)));
  return sections.filter((section): section is TidalBrowseSection => Boolean(section));
}
