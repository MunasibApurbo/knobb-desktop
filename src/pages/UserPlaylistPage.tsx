...
import { Play, Pause, Shuffle, Trash2, Music, Loader2, Share, Heart } from "lucide-react";
...
export default function UserPlaylistPage() {
  ...
  const { playlists, loading, removeTrack, deletePlaylist } = usePlaylists();
  ...
  const coverUrl = playlist.cover_url || playlist.tracks[0]?.coverUrl || "/placeholder.svg";
  ...
  const handleLikeAll = async () => {
    if (playlist.tracks.length === 0) return;
    const tracksToLike = playlist.tracks.filter((track) => !isLiked(track.id));
  ...
  const handleSharePlaylist = async () => {
    ...
    const text = playlist.name.trim();
    if (navigator.share) {
      try {
        await navigator.share({ title: playlist.name, text, url });
        return;
      } catch {
    ...
    <PageTransition>
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.3 }} className="space-y-0">
            ...
            <div className="relative overflow-hidden mb-0 border border-white/10 border-b-0" style={{ height: "400px" }}>
              ...
              <div className="relative h-full z-[2] flex items-end">
                <div className="absolute top-0 right-0 bottom-0 w-full sm:w-[65%] shrink-0 z-0">
                  {playlist.cover_url || playlist.tracks[0]?.coverUrl ? (
                    <img
                      src={coverUrl}
                      alt={playlist.name}
                ...
                <div className="relative z-10 w-full sm:w-[60%] flex flex-col justify-end px-8 md:px-10 pb-8 min-w-0 pointer-events-none">
                  <div className="pointer-events-auto">
                    ...
                    <div className="text-xs font-medium text-foreground/60 tracking-wide uppercase mt-1" style={{ opacity: scrollOpacity }}>
                      {playlist.tracks.length} TRACK{playlist.tracks.length !== 1 ? "S" : ""} {playlist.tracks.length > 0 ? `(${getTotalDuration(playlist.tracks)})` : ""}
                    </div>
                  </div>
                </div>
              </div>
        ...
        <section className="grid grid-cols-2 md:grid-cols-4 border border-white/10 divide-x divide-y md:divide-y-0 divide-white/10 bg-white/[0.02]">
          <Button variant="secondary" className={actionBtnClass} onClick={handlePlayAll} disabled={playlist.tracks.length === 0}>
          ...
          <Button variant="secondary" className={actionBtnClass} onClick={handleShuffle} disabled={playlist.tracks.length === 0}>
            <Shuffle className="w-4 h-4 mr-2" />
            <span className="relative z-10">Shuffle</span>
          </Button>
          <Button variant="secondary" className={actionBtnClass} onClick={handleLikeAll} disabled={playlist.tracks.length === 0}>
        ...
        <section className="border border-t-0 border-white/10 bg-white/[0.02] px-4 py-2 flex justify-end">
          <button
            type="button"
            className="text-xs uppercase tracking-wide font-semibold text-red-400/80 hover:text-red-300 transition-colors"
        ...
        {playlist.tracks.length > 0 ? (
            ...
            <div>
              {playlist.tracks.map((track, i) => {
                ...
                return (
                  <button
                    key={`${track.id}-${i}`}
                    className={`group relative overflow-hidden w-full grid grid-cols-[36px_48px_minmax(0,1fr)_72px_72px] md:grid-cols-[36px_48px_minmax(0,1fr)_minmax(0,0.8fr)_72px_72px] gap-3 px-4 py-2.5 items-center text-left border-b last:border-b-0 border-white/10 ${isCurrent ? "" : "transition-colors duration-200"}`}
                  ...
                  >
                    ...
                    <span className="relative z-10 flex items-center justify-center gap-1 opacity-0 invisible group-hover:opacity-100 group-hover:visible focus-within:opacity-100 focus-within:visible transition-opacity duration-200">
                      <button
                        type="button"
                      ...
                      >
                        <Heart
                          className={`w-4 h-4 ${isLiked(track.id)
                      ...
                      <button
                        ...
                        className={`inline-flex items-center justify-center w-8 h-8 ${isCurrent ? "text-[hsl(var(--dynamic-accent-foreground))]" : "text-muted-foreground group-hover:text-[hsl(var(--dynamic-accent-foreground))] transition-colors duration-200"}`}
                        onClick={(event) => {
                          event.preventDefault();
                          event.stopPropagation();
                          void removeTrack(playlist.id, i);
                        }}
                      >
          ...
          <section className="border border-t-0 border-white/10 bg-white/[0.02]">
            <p className="text-muted-foreground text-sm py-10 text-center">No tracks yet. Add songs from search or artist pages.</p>
          </section>
        )}
      </motion.div>
    </PageTransition>
