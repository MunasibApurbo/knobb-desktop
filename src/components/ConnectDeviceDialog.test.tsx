...
describe("ConnectDeviceDialog", () => {
  ...
  it("supports controlled open state without rendering the trigger button", () => {
    render(<ConnectDeviceDialog open hideTrigger onOpenChange={vi.fn()} />);

    expect(screen.queryByTitle("Connect to a device")).not.toBeInTheDocument();
    expect(screen.getByRole("dialog")).toBeVisible();
    expect(screen.getByText("Connect")).toBeInTheDocument();
  });
});
