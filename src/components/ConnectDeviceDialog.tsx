    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button
          variant="ghost"
          size="icon"
          className={`player-chrome-utility h-9 w-9 bg-white/[0.035] transition-colors ${
            open || isOnExternalDevice
              ? "text-[hsl(var(--player-waveform))] hover:bg-white/[0.085]"
              : "text-white/68 hover:bg-white/[0.085] hover:text-white"
          }`}
      ...
      <DialogContent className="max-w-md overflow-hidden p-0 backdrop-blur-2xl">
        <div className="border-b border-white/10 px-6 py-5">
          <DialogHeader className="space-y-2 text-left">
            <DialogTitle className="text-2xl font-bold tracking-tight">Connect</DialogTitle>
        ...
        <div className="space-y-5 px-6 py-5">
          <button
            ...
            className="group relative w-full overflow-hidden border border-white/10 bg-white/[0.04] p-5 text-left transition-colors hover:border-white/20"
          >
            <span
          ...
          {user ? (
            <div className="space-y-2">
              ...
              {isLoadingRemoteSessions ? (
                <div className="flex items-center gap-3 border border-white/10 bg-white/[0.02] px-4 py-3 text-sm text-muted-foreground">
                ...
                remoteSessions.map((session) => {
                  ...
                  return (
                    <div
                      key={session.id}
                      className="flex items-center gap-4 border border-white/10 bg-white/[0.02] px-4 py-3"
                    >
                      <div className="flex h-10 w-10 items-center justify-center border border-white/10 bg-black/20 text-muted-foreground">
                      ...
                      <Button
                        variant="outline"
                        className="h-9 border-white/12 bg-white/[0.03] px-3 text-xs font-semibold text-foreground hover:bg-white/[0.05]"
                ...
                <div className="border border-white/10 bg-white/[0.02] px-4 py-3 text-sm text-muted-foreground">
                  No other signed-in Knobb sessions are active right now.
                </div>
              )}
            </div>
          ) : (
            <div className="flex gap-4 border border-white/10 bg-white/[0.02] px-4 py-3">
              ...
              {otherDevices.map((device) => {
                ...
                return (
                  <button
                    ...
                    disabled={isSwitching !== null}
                    className={`flex w-full items-center gap-4 border border-white/10 px-4 py-3 text-left transition-colors ${
