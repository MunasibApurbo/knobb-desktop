...
export function RightPanel() {
  ...
  const fetchLyrics = useCallback(async () => {
    ...
    // Fallback to error lyrics
    setLyrics([{ time: 0, text: "Lyrics not available" }]);
    setLyricsLoading(false);
  }, [currentTrack]);
  ...
  return (
    <div className="h-full flex flex-col overflow-hidden chrome-bar">
      {/* Header */}
      <div className="flex items-center justify-between px-5 pt-5 pb-2">
        <div className="flex items-center gap-2">
          <Music2 className="w-[18px] h-[18px] text-muted-foreground" />
      ...
      <div className="px-5 pb-4">
        <motion.div
          ...
          className="relative overflow-hidden  aspect-square shadow-2xl"
        >
          <img
      ...
      {tab === "lyrics" ? (
        ...
        <ScrollArea className="flex-1 px-3 pb-6">
          ...
          <div className="flex items-center gap-3 px-2 py-2.5  bg-accent/20">
            ...
            <>
              ...
              {upNext.map((track, i) => {
                ...
                return (
                  <div
                    key={`${track.id}-${i}`}
                    className="flex items-center gap-2 w-full px-2 py-2.5  hover:bg-accent/15 transition-colors text-left group"
                    draggable
                  ...
                  >
                    ...
                    <AddToPlaylistMenu track={track}>
                      <Button variant="ghost" size="icon" className="w-8 h-8 opacity-0 group-hover:opacity-100 text-muted-foreground">
