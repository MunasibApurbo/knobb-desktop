...
export const SidebarLibraryCard = forwardRef<HTMLButtonElement, SidebarLibraryCardProps>(function SidebarLibraryCard({
  ...
  if (layout === "list") {
    return (
      <button
        ...
        className={`sidebar-library-card content-visibility-tile group relative flex w-full items-center gap-3 overflow-hidden px-3 py-2.5 text-left transition-colors duration-200 ${
          isDropTarget
      ...
      >
        <div
          className={`sidebar-library-media relative h-14 w-14 shrink-0 overflow-hidden border border-white/10 bg-white/[0.04] ${
        ...
        <div className="min-w-0 flex-1">
          <p
            className={`truncate text-[1.02rem] font-semibold leading-tight tracking-tight transition-colors duration-200 ${
              isHighlighted ? "text-black" : "text-foreground group-hover:text-black"
            }`}
          >
            {title}
          </p>
          {subtitle ? (
            <p
              className={`mt-1 truncate text-[0.92rem] leading-tight transition-colors duration-200 ${
                isHighlighted ? "text-black/70" : "text-white/58 group-hover:text-black/65"
              }`}
            >
              {subtitle}
            </p>
  ...
  return (
    <button
      ...
      className={`sidebar-library-card content-visibility-tile group relative block w-full overflow-hidden text-left transition-all duration-200 ${isArtist ? "h-[84px]" : "h-24"} ${
        isDropTarget
    ...
    >
      ...
      <div
        className={`absolute inset-0 transition-opacity duration-200 ${
          isHighlighted ? "bg-gradient-to-r from-black/72 via-black/35 to-black/18" : "bg-gradient-to-r from-black/80 via-black/45 to-black/20"
        }`}
      />
      <div
        className={`pointer-events-none absolute inset-y-0 left-0 w-[58%] bg-gradient-to-r to-transparent transition-opacity duration-200 ${
          isHighlighted ? "from-[hsl(var(--player-waveform)/0.28)] via-black/22" : "from-black/50 via-black/20"
        }`}
      />
      {active ? (
      ...
      <div className={`absolute left-4 right-4 ${isArtist ? "bottom-2" : "bottom-2.5"}`}>
        <p
          className={`truncate text-foreground/95 drop-shadow-[0_2px_4px_rgba(0,0,0,0.8)] ${
            isArtist ? "text-[1.45rem]" : "text-[1.2rem]"
