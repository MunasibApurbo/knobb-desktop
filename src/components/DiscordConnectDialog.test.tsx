...
describe("DiscordConnectDialog", () => {
  ...
  it("renders desktop config actions when Knobb Desktop is available", async () => {
    window.knobbDesktop = {
      isDesktopApp: true,
      platform: "darwin",
      getLaunchTarget: vi.fn().mockResolvedValue(null),
      openExternal: vi.fn().mockResolvedValue(true),
      getConfigDirectory: vi.fn().mockResolvedValue("/Users/test/Library/Application Support/Knobb"),
      getConfigFilePath: vi.fn().mockResolvedValue("/Users/test/Library/Application Support/Knobb/discord-presence.bridge.json"),
      openConfigDirectory: vi.fn().mockResolvedValue(true),
      revealConfigFile: vi.fn().mockResolvedValue(true),
    };
    ...
    expect(await screen.findByText("Knobb Desktop is already running")).toBeInTheDocument();
    expect(screen.getByText("Copy config path")).toBeInTheDocument();
    expect(screen.getByText("Open config folder")).toBeInTheDocument();
    expect(screen.getByText("Reveal config file")).toBeInTheDocument();
  });
});
